...
contract electricityBillingConcract {
    ...
    function processBillingForEnergy() public checkIfProcessBillingAlradyMade
    {
        ...
        {   
            if (usrActive[usrAddress[i]]==true)
            {
                int usrProPriceNanoCent=((puPro*nS1Ws*usrEdSr[usrAddress[i]])/N)+((nS2Ws*usrEbAvSr[usrAddress[i]])/N);
                int usrConPriceNanoCent=((puCon*nB1Ws*usrEdLd[usrAddress[i]])/N)+((nB2Ws*usrEbAvLd[usrAddress[i]])/N)+((puCon*nB1Ws*usrEbRqLd[usrAddress[i]])/N);
